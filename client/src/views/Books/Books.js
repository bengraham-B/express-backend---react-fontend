...
class FavObject {
	constructor(index, title,wrapperType, artistName, trackCensoredName, kind, releaseDate, primaryGenreName, country, trackViewUrl){
		this.index = index
		this.title = title
...
export default function Itunes() {
	...
	const searchItunes = () => {
		...
		if(document.getElementById("itunes-search").value){
          ...
          console.log("wrapperType",data.results[i].wrapperType)

          if(data.results[i].wrapperType){
      
                resultsArray.push(
                  {
					artistName: data.results[i].artistName,
					wrapperType: data.results[i].wrapperType,
	...
	const hanldeFav = (i) => {
		...
		if(localStorage.getItem("itunes-fav") === null){
			...
			console.log("Create Empty Local Storage Key: books-fav")
			...
			localStorage.setItem("books-fav", JSON.stringify(addArray))
	...
	return (
		<div id="ITUNES">
			...
			<div className="title-container">
				<h1 className="title">Search Apple Books</h1>
			</div>
